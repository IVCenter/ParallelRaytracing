...
public class SeethroughSphereTest extends Scene
{	
	/*
	 * A simple test scene for cse167 project3 inspirations (2015)
	 */
	...
	{				
		Material skyMaterial = new ColorMaterial((new Color(0xffffffff)).multiply3(1.6));
		return skyMaterial;
	}
	...
	protected void configureWorld()
	{
		...
		s1 = new Sphere(1.0, new Vector3(-1.5, 2.0, 0.0));
		s1.setMaterial(new PassThroughMaterial(new Color(0xff61c8ff)));
		surfaces.add(s1);
		
		s2 = new Sphere(1.0, new Vector3(1.5, 2.0, 0.0));
		s2.setMaterial(new PassThroughMaterial(new Color(0xc8ff61ff)));
		surfaces.add(s2);
		...
		if(dragon != null) {
			...
			dragon.setMaterial(new PassThroughMaterial(new Color(0x61c8ffff)));
		...
		DiffuseMaterial dmat = new DiffuseMaterial(new Color(0xbbbbbbff));
		
		grid.setMaterial(new ColorMaterial(new Color(0xaaaaaaff)));
		...
		this.addChildrenUnsafe(surfaces);
	}
	...
	public void update(UpdateData data)
	{
		...
		m.identity();
		m.rotateY(data.getDt());

		s1.getPosition().set(m.multiplyPt(s1.getPosition()));
		s2.getPosition().set(m.multiplyPt(s2.getPosition()));
	...
	private static void loadDebugConfiguration()
	{
		...
		Configuration.setRenderWidth(512);
		Configuration.setRenderHeight(512);
		...
		Configuration.setMasterScene(new SeethroughSphereTest());
	}
}